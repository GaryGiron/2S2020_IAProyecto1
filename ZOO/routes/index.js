...
mongoose.connect('mongodb://35.232.42.57:27017/animales',{useNewUrlParser: true}).then(()=>{
...
vertebrados(Y) :- pertenece_a(peces,X),animal(X,Y,_,_,_,_,_,_,_).
vertebrados(Y) :- pertenece_a(anfibios,X),animal(X,Y,_,_,_,_,_,_,_).
vertebrados(Y) :- pertenece_a(aves,X),animal(X,Y,_,_,_,_,_,_,_).
vertebrados(Y) :- pertenece_a(mamiferos,X),animal(X,Y,_,_,_,_,_,_,_).
...
router.get('/', function(req, res, next) {
  ...
  success: function() {
    ...
    session.query(goal, {
      success: function() {
        // Look for answers
        session.answers(Y => console.log(pl.format_answer(Y)));
        res.render('index', { title: 'Zoologico la Aurora' });
      }
    });
  }
  });
